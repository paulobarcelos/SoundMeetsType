...
function (
	...
	var FrequencyData = function(fftSize, audioContext, destination) {
		...
		var update = function() {
			...
			if(averageSum < 0.05) averageSum = 0.05;
	...
	var BeatDetector = function(audioContext, destination, fftSize){
		...
		var init = function () {
			...
			GlobalGui.add('Advanced', 'Debug Spectrum', false);
		...
		var addFilter = function (name) {
			...
			GlobalGui.add('Advanced', name + ' Filter Frequency', 440, 0, 18000);
			GlobalGui.add('Advanced', name + ' Filter Q', 0, 0, 150);
			GlobalGui.add('Advanced', name + ' Filter Gain', 1, 0, 40);
